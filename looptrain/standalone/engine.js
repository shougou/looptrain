...
var FALLBACK_NPCS = {
  xiaoning: {
    ...
    portrait: 'xiaoning_portrait.png', role: 'clue_source', location: 'carriage_2',
  ...
  zhao_police: {
    ...
    portrait: 'zhao_police_portrait.png', role: 'authority_gatekeeper', location: 'carriage_2',
    ...
    suggestions: [
      { label: '报告异常', template: '我压低声音告诉赵乘警：二号车厢有异常声音，我需要你检查地板。' },
      { label: '展示证据', template: '我把目前获得的线索逐条告诉赵乘警，请他亲自听一听地板下方的声音。' },
      { label: '请求检查地板', template: '我请求赵乘警不要惊动乘客，只检查二号车厢地板和连接处。' },
      { label: '结束对话', template: '__END_DIALOGUE__' },
    ],
  },
  shen_mohan: {
...
function parseAction(text) {
  ...
  if (/回到二号车厢|返回二号车厢|回到车厢|返回车厢|回车厢/.test(t)) return { intent: 'move_to_carriage_2', confidence: 0.88 };
...
function suggestions(s) {
  ...
  if (loc === 'carriage_2') {
    ...
    out.push(
      ...
      { label: '找赵乘警', template: '我找到赵乘警，压低声音报告二号车厢的异常。' },
    );
  }
...
function startDialogue(state, npcId) {
  var s = normalize(state);
  var npc = NPCS[npcId];
  if (!npc) return { state: s, messages: [{ type: 'system', text: '这个人现在不在二号车厢。' }], suggestions: suggestions(s), goal: currentGoal(s) };
...
function commitAction(text, state) {
  ...
  if (action.intent === 'move_to_carriage_2') {
    ...
    return { state: s, messages: [{ type: 'system', text: '你从连接处回到二号车厢。过道里灯光明暗不定，小宁还在靠窗的位置，赵乘警仍在查票。地板下方的滴答声依然隐约可闻。' }], suggestions: suggestions(s), goal: currentGoal(s) };
  }
