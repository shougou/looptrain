...
import type { GoalCondition, GoalRuntimeState } from './GoalTypes';
...
function eventMatches(
  cond: { type: string; npcId?: string; topic?: string; result?: string },
  event: MemoryEvent
): boolean {
  const evt = event as unknown as Record<string, unknown>;
  if (evt.type !== cond.type) return false;
  const payload = (evt.payload as Record<string, unknown>) || {};
  if (cond.npcId !== undefined && payload.npcId !== cond.npcId) return false;
  if (cond.topic !== undefined && payload.topic !== cond.topic) return false;
  if (cond.result !== undefined && payload.result !== cond.result) return false;
  return true;
}
...
export function evaluateCondition(
  ...
  // ── Combinators ──────────────────────────────────────────
  if ('all' in condition) {
    if (condition.all.length === 0) return true; // vacuous truth
