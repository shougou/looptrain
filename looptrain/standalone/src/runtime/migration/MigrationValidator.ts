...
export function validateMigration(
  ...
  for (const clueId of legacy.known_clues) {
    const found = state.knowledge.some(
      (k) => k.clueId === clueId || k.title === clueId
    );
    if (!found) {
  ...
  if (legacy.carried_memory && legacy.carried_memory.length > 0) {
    const hasCarryover = state.knowledge.some((k) =>
      legacy.carried_memory.some((cm) => k.title.includes(cm) || k.summary.includes(cm))
    );
    if (!hasCarryover) {
