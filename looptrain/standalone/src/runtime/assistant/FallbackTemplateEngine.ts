...
function getText(intent: AssistantIntent, view: CompanionView): string {
  ...
  switch (intent) {
    case 'ASK_NEXT_ACTION':
      if (loopNum <= 0) return `欢迎来到${sceneLabel}。我是许知微，会协助你进行调查。你可以先观察周围环境，或者尝试与在场的人交谈。`;
    ...
    case 'ASK_CLUE_SUMMARY':
      if (view.knowledge.confirmedClueIds.length === 0) return '目前还没有确认的线索。建议先观察环境或与人交谈。';
      return '目前可以确认的是：我们掌握了一些线索。建议继续深入调查。';

    case 'ASK_LOOP_SUMMARY':
      if (view.timeline.previousLoops.length === 0) return '这是第一轮调查，还没有之前的记录可以回顾。';
