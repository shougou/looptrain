...
let _responses: MockResponses | null = null;

function getResponses(): MockResponses {
  if (_responses) return _responses;
  try {
    const loader = new RuntimeContentLoader();
    _responses = loader.loadRuntimeJSON<MockResponses>('assistant/mock-responses.json');
  } catch (_) {
...
function getHardcodedResponses(): MockResponses {
  return {
...
export class MockLLMProvider implements LLMProvider {
  ...
  private getMockText(mode: string): string {
