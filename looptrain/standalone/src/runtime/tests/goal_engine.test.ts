...
function mkState(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    events: (overrides.events as unknown[]) || [],
    knownClues: (overrides.knownClues as string[]) || [],
    confirmedFacts: (overrides.confirmedFacts as string[]) || [],
...
{
  const result = runtime.evaluateCondition(
    { clueKnown: 'test' },
    mkState({ knownClues: ['test'] })
  );
...
{
  const result = runtime.evaluateCondition(
    { factConfirmed: 'fact_a' },
    mkState({ confirmedFacts: ['fact_a'] })
  );
...
{
  const result = runtime.evaluateCondition(
    { stateEquals: { key: 'loop', value: 1 } },
    mkState({ state: { loop: 1 } })
  );
...
{
  const result = runtime.evaluateCondition(
    { all: [{ clueKnown: 'a' }, { clueKnown: 'b' }] },
    mkState({ knownClues: ['a', 'b'] })
  );
...
{
  const result = runtime.evaluateCondition(
    { not: { clueKnown: 'present' } },
    mkState({ knownClues: ['present'] })
  );
...
{
  const result = runtime.evaluateCondition(
    { eventOccurred: { type: 'dialogue_end', npcId: 'xiaoning' } },
    mkState({ events: [{ type: 'dialogue_end', payload: { npcId: 'xiaoning' } }] })
  );
  assert.strictEqual(result, true, 'eventOccurred: true when event matches');
}
{
  const result = runtime.evaluateCondition(
    { eventOccurred: { type: 'dialogue_end', npcId: 'zhao_police' } },
    mkState({ events: [{ type: 'dialogue_end', payload: { npcId: 'xiaoning' } }] })
  );
...
// ── Test 9: GoalEngine newlyCompleted triggers only once ───────
{
  const def = {
    id: 'goal.test_once',
    title: 'Test Once',
    completionCondition: { clueKnown: 'key_clue' } as const,
    loopRange: [1, 5] as [number, number],
  };
...
// ── Test 10: activationCondition gate ──────────────────────────
{
  const def = {
    ...
    activationCondition: { clueKnown: 'unlock' } as const,
    completionCondition: { clueKnown: 'key_clue' } as const,
  };
...
// ── Test 11: loopRange gate ────────────────────────────────────
{
  const def = {
    ...
    completionCondition: { clueKnown: 'key_clue' } as const,
    loopRange: [3, 5] as [number, number],
  };
