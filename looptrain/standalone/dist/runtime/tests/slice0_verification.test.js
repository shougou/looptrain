...
// ── IntentClassifier ───────────────────────────────────────
assert.strictEqual(runtime.classifyIntent('ASK_ASSISTANT_BUTTON'), 'ASK_NEXT_ACTION', 'empty button click must classify as ASK_NEXT_ACTION');
assert.strictEqual(runtime.classifyIntent('ASK_ASSISTANT_BUTTON', '真凶是他'), 'ASK_TRUTH', 'truth-seeking text must classify as ASK_TRUTH');
...
// ── OutputValidator ─────────────────────────────────────────
const mockResponse = function (text) { return { mode: 'deterministic_template', visibleText: text, actionRefs: [], clueRefs: [], beliefRefs: [], spoilerLevel: 0, confidence: 'medium', stateEffects: [] }; };
